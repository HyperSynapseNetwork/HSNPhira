...
# 确保 twa-manifest.json 文件存在
if [ ! -f "twa-manifest.json" ]; then
    echo "📄 创建 twa-manifest.json 配置文件..."
    cp "$PROJECT_ROOT/twa/twa-manifest.json" .
    echo "✅ twa-manifest.json 已创建"
else
    echo "📄 使用现有的 twa-manifest.json 配置文件"
fi
...
if [ -z "$TWA_KEYSTORE_BASE64" ] || [ -z "$TWA_KEYSTORE_PASSWORD" ] || \
   [ -z "$TWA_KEY_ALIAS" ] || [ -z "$TWA_KEY_PASSWORD" ]; then
    ...
    if echo "$CLEAN_BASE64" | base64 -d > /dev/null 2>&1; then
        ...
        echo "✅ 密钥库文件已解码: production.keystore"
        
        # 更新 twa-manifest.json 中的签名配置（如果支持）
        if command -v jq &> /dev/null; then
            echo "🔧 更新签名配置..."
            jq '.signing = {
                "path": "production.keystore",
                "alias": "'"$TWA_KEY_ALIAS"'"
